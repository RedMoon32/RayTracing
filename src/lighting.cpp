		...
		for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
			...
			for (size_t v = 0; v < fv; v++) {
				...
				if (idx.normal_index >= 0)
				{
					...
					auto new_ = Vertex(float3{ vx, vy, vz }, float3{ nx, ny, nz });
					vertexes.push_back(new_);
				}
				else
				{
...
Payload Lighting::Hit(const Ray& ray, const IntersectableData& data, const MaterialTriangle* triangle) const
{
	...
	for (auto& light : lights)
	{
		...
		payload.color += light->color * triangle->diffuse_color * std::max(dot(N, toLight.direction), 0.f);
